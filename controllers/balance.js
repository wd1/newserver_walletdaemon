...
import { getAddressBalances } from '../services/balanceChecker';
    ...
    try {
        ...
        await Promise.all(accounts.map(async account => {
            ...
            return Promise.all(Object.keys(balances).map(async tokenAddr => {
                const coinIdx = (tokenAddr === '0x0') ? coins[coinEthIdx]._id : coins.findIndex(coin => coin.address === tokenAddr);

                let wallet = await Wallets.findOne({accountId: account._id, coinId: coinIdx});
