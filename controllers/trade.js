    ...
    events.forEach(async event => {
        ...
        if (openOrders && openOrders.length > 0) {
            try {
                ...
                const account = accounts.find(account => account.beneficiary === tradeAddress);
                const cryptoIds = [];
                const quantities = [];
                const prices = [];

                if (filledOrder === -1 && account) {
                    if (params.length > 3) {
                        ...
                        if (tradeType === 'asset') {
                            ...
                            if (coin) {
                                const order = _.find(openOrders, order =>
                                    order.coinId == coin._id &&
                                    order.accountId == account._id &&
                                    Number(order.quantity).toFixed(8) == Number(quantities[0]).toFixed(8) &&
                                ...
                                if (index) {
                                    ...
                                    if (indexContains && indexContains.length === cryptoCount) {
                                        ...
                                        for (let i = 0; i < cryptoIds.length; i++) {
                                            ...
                                            if (coin) {
                                                const idxContain = indexContains.find(ic => ic.coinId == coin._id && Number(ic.quantity).toFixed(8) === Number(quantities[j]).toFixed(8));
                                                match = !!idxContain;
                                                if (!idxContain) break;
                                            }
                                        }
